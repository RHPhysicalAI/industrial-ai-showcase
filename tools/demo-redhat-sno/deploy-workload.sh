...
oc create configmap mission-dispatcher-config -n warehouse-edge \
  --from-literal="kafka.bootstrap.servers=${HUB_KAFKA_ROUTE}:443" \
  --dry-run=client -o yaml | oc apply -f -
oc create configmap mission-dispatcher-config -n robot-edge \
  --from-literal="kafka.bootstrap.servers=${HUB_KAFKA_ROUTE}:443" \
...
oc --context="$HUB_CONTEXT" get secret "$hub_kafka_ca_secret" \
  ...
  > "$ca_file"
oc create secret generic "$companion_secret" -n "$companion_namespace" \
  --from-file=ca.crt="$ca_file" --dry-run=client -o yaml | oc apply -f -

printf '%s\n' 'Refreshing workloads after the CA/configuration update...'
oc rollout restart deployment/fake-camera -n warehouse-edge
oc rollout restart deployment/mission-dispatcher -n robot-edge
