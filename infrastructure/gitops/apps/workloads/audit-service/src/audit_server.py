...
async def mark_merge_failed(approval_id: int, request: MergeFailedRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to mark merge failed")
    finally:
        conn.close()
...
async def attach_moderation_results(approval_id: int, moderation_data: dict):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to attach moderation results")
    finally:
        conn.close()
    ...
    try:
        ...
        if session_id:
            cursor.execute(
                ...
                FROM hil_audit
                WHERE session_id = %s AND approval_status IN ('approved', 'rejected', 'merge_failed')
            ...
            cursor.execute(
                ...
                SELECT id, timestamp, session_id, user_identity, tool_name,
                       ...
                       approver_identity, rejection_reason, result
                FROM hil_audit
                WHERE approval_status IN ('approved', 'rejected', 'merge_failed')
                ORDER BY timestamp DESC
                LIMIT %s
                """,
                (limit,)
